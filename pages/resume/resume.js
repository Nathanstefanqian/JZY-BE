...
Page({
  ...
  async onLoad(e) {
    ...
    res = res.data[0]
    // 进行url转换
    res.photoUrl = res.photoUrl.map(item => reg(item))
    res.skillList = res.skillList.map(item => {
      console.log('列表', item.fileList)
      item.fileList = item.fileList.map(subitem => {
        subitem.url = reg(subitem.url)
        return subitem
      })
      return item
    })
    res.avatarUrl = reg(res.avatarUrl)
    this.setData({
      sex: res.sex,
      ...
      jobLookingFor: res.job ? res.job.isPeriod + ' ; ' + res.job.worktime + ' ; ' + res.job.workday + ' ; ' + res.job.workperiod + ' ; ' + res.job.name + ' ; ' : '暂无'
    }, () => wx.hideLoading())
  },
  // 表单输入方法
  onPersonalChange(e) {
  ...
  onSkillPreview(e) {
    ...
    console.log(skill)
    let skillItem = this.data.skill
    console.log(skillItem)
    const index = skillItem.fileList.findIndex(item => item.url == skill.url)
    console.log('序号', index)
    let { fileList } = skillItem
    fileList = fileList.map(item => item.url)
    wx.previewImage({
      current: fileList[index],
      urls: fileList // 当前显示图片的链接
    })
  },
  onAvatarPreview() {
